...
export class MemoProvider implements vscode.TreeDataProvider<MemoTree>{
    private _onDidChangeTreeData: vscode.EventEmitter<MemoTree | undefined> = new vscode.EventEmitter<MemoTree | undefined>();
    ...
    getChildren(element?: MemoTree): Thenable<MemoTree[]> {
        if (element) {
            ...
            return Promise.resolve(files.map(f => {
                const tree = new MemoTree(f.label, f.id, f.paths, vscode.TreeItemCollapsibleState.None, { title: 'memoFilePreview', command: MemoFilePreviewCommand, arguments: [f.param] })
                tree.contextValue = MemoFileContext
                return tree
            }))
        } else {
            const folders = this.getFolders()
            return Promise.resolve(folders.map(f => {
