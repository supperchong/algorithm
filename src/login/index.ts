...
async function saveResCookie(cookies: string[] = []) {
    ...
    const cookieStr = Object.keys(map).map(key => `${key}=${map[key]}`).join('; ')
...
type RequestAsync = (arg1: (_request.UriOptions & _request.CoreOptions) | (_request.UrlOptions & _request.CoreOptions)) => Promise<_request.Response>
