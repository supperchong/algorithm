...
async function inputName(input: MultiStepInput, user: Partial<User>, title: string): Promise<InputStep> {
    user.name = await input.showInputBox({ step: 1, totalSteps: 2, validate: async () => '', prompt: 'please input username or email', title, value: '', shouldResume: () => Promise.resolve(false) })
    return input => inputPass(input, user, title)
}
async function inputPass(input: MultiStepInput, user: Partial<User>, title: string) {
    user.password = await input.showInputBox({ step: 2, totalSteps: 2, password: true, validate: async () => '', prompt: 'please input password', title, value: '', shouldResume: () => Promise.resolve(false) })
}
...
export async function cookieInput() {
...
class MultiStepInput {
    ...
    async showInputBox<P extends InputBoxParameters>({ title, step, totalSteps, password = false, value, prompt, validate, buttons, shouldResume }: P) {
        const disposables: Disposable[] = [];
        try {
